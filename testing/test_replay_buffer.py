...
class TestReplayBuffer:
    ...
    def test_add_trajectories(self, simple_env, trajectories):
    ...
    def test_add_transitions(self, simple_env, transitions):
        buffer = ReplayBuffer(simple_env, capacity=10)
    ...
    def test_add_state_pairs(self, simple_env, state_pairs):
        buffer = ReplayBuffer(simple_env, capacity=10)
    ...
    def test_capacity_limit(self, simple_env, trajectories):
    ...
    def test_prioritized(self, simple_env, trajectories):
        ...
        assert torch.allclose(buffer.training_objects.log_rewards, torch.tensor([0.0, 1.0, 2.0, 3.0, 4.0]))
    
    def test_sample(self, simple_env, trajectories):
        buffer = ReplayBuffer(simple_env, capacity=10)
        buffer.add(trajectories)
    ...
    def test_save_load(self, simple_env, trajectories):
        buffer = ReplayBuffer(simple_env, capacity=10)
        buffer.add(trajectories)
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            assert torch.allclose(
                new_buffer.training_objects.log_rewards,
                buffer.training_objects.log_rewards
            )
    
    def test_type_error(self, simple_env):
        buffer = ReplayBuffer(simple_env)
        
        # Try to add an invalid type
        with pytest.raises(TypeError, match="Must be a container type"):
            buffer.add("not a container")


class TestNormBasedDiversePrioritizedReplayBuffer:
    def test_init(self, simple_env):
    ...
    def test_add_with_diversity(self, simple_env, trajectories):
        buffer = NormBasedDiversePrioritizedReplayBuffer(
            simple_env,
            capacity=5,
            cutoff_distance=0.5,
    ...
    def test_skip_diversity_check(self, simple_env, trajectories):
