...
def trajectory_balance_loss(env, pf, pb, logZ, traj, actions, reward, MIN_REW=1e-5):
    ...
    :param reward: reward at the end of the trajectory as a tensor
    :return: loss(traj)
    ...
    forward_logprobs = torch.gather(
        trajectory_logits, 1, actions.unsqueeze(1)).squeeze(1)
    ...
    log_PB_actions = torch.gather(
        log_PB_all, 1, actions[:-1].unsqueeze(1)).squeeze(1)
...
def online_TB_loss(env, pf, pb, logZ, start_states, loss_fn=nn.MSELoss(),
    ...
    while torch.any(~dones):
        ...
        if actions is not None:
            ...
            log_PB_actions = torch.gather(
                log_PB_all, 1, actions[~over].unsqueeze(1)).squeeze(1)
        ...
        log_PF_actions = torch.gather(
            log_PF_all, 1, actions.unsqueeze(1)).squeeze(1)
