...
Note: The numerical implementation has been improved from the original published
version. Key changes from the reference code:
- arccos replaced with atan2 for improved numerical stability
- Removed double-precision (torch.double) casts that were previously needed
- BoxPFMLP.forward rewritten with torch.where for torch.compile compatibility

The mathematical semantics are unchanged: actions are polar vectors with L2 norm
equal to delta, parameterised via quarter-circle Beta mixtures.
"""
