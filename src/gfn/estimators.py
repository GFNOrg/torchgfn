...
class Estimator(ABC, nn.Module):
    ...
    This class is the base class for all estimators. The estimators need to encapsulate a
    `GFNModule`, which takes a batch of states as input and outputs a batch of outputs
    of the desired shape. When the goal is to represent a probability distribution, the
    outputs would correspond to the parameters of the distribution, e.g. logits for a
    categorical distribution for discrete environments.
    ...
    Attributes:
        module: The GFNModule object to use. If it is a Tabular module (from
    ...
    def forward(self, input: States | torch.Tensor) -> torch.Tensor:
        ...
        Args:
            input: The input to the estimator, as states or a tensor.

        Returns:
            The output of the estimator, as a tensor of shape (*batch_shape, output_dim).
        """
    ...
    def expected_output_dim(self) -> Optional[int]:
        """Expected output dimension of the estimator.

        Returns:
            The expected output dimension of the estimator, or None if the output
            dimension is not well-defined (e.g., when the output is a TensorDict for
            GraphActions).
        """
    ...
    def to_probability_distribution(
        ...
        **policy_kwargs: Any,
        ...
        """Transforms the output of the estimator into a probability distribution.

        The kwargs may contain parameters to modify a base distribution, for example to
        encourage exploration.

        This method is optional for estimators that don't need to output probability
        distributions (e.g., when estimating logF for flow matching). However, it is
        required for estimators that define policies, as it converts raw estimator outputs
        into probability distributions over actions. See `DiscretePolicyEstimator` for an
        ...
        Args:
            ...
            module_output: The output of the estimator as a tensor of shape
...
class ScalarEstimator(Estimator):
    ...
    Attributes:
        module: The GFNModule to use. This doesn't have to directly output a
            scalar. If it does not, `reduction` will be used to aggregate the outputs of
            the GFNModule into a single scalar.
        preprocessor: Preprocessor object that transforms raw States objects to tensors
    ...
    def forward(self, input: States | torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            The output of the estimator, as a tensor of shape (*batch_shape, 1).
        """
        # If the input is a States object, preprocess it.
        out = self.preprocessor(input) if isinstance(input, States) else input
        out = self.module(out)
...
class ConditionalDiscretePolicyEstimator(DiscretePolicyEstimator):
    ...
    Attributes:
        state_module: The GFNModule for state processing.
        conditioning_module: The GFNModule for conditioning processing.
        final_module: The GFNModule that combines state and conditioning.
        n_actions: Total number of actions in the discrete environment.
        preprocessor: Preprocessor object that transforms raw States objects to tensors.
        is_backward: Flag indicating whether this estimator is for backward policy,
            i.e., is used for predicting probability distributions over parents.
    ...
    def __init__(
        self,
        state_module: GFNModule,
        conditioning_module: GFNModule,
        final_module: GFNModule,
        n_actions: int,
        preprocessor: Preprocessor | None = None,
        is_backward: bool = False,
    ):
...
class DiscreteGraphPolicyEstimator(Estimator):
    ...
    def forward(self, input: States | torch.Tensor | GeometricBatch) -> torch.Tensor:
        ...
        Args:
            input: The input to the estimator, as states, tensor, or GeometricBatch.
    ...
    def to_probability_distribution(
        ...
        """Returns a probability distribution given a batch of states and estimator output.
        ...
        Args:
            ...
            module_output: The output of the estimator as a TensorDict containing logits
