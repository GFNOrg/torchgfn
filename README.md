...
+ [Losses](https://torchgfn.readthedocs.io/en/latest/guides/losses.html): Each type of `GFlowNet` has a specific parameterization related to a specific loss. Covers TB, DB, SubTB, FM, ZVar, and RTB with guidance on when to use each.
+ [Off-Policy Training & Replay Buffers](https://torchgfn.readthedocs.io/en/latest/guides/off_policy_training.html): How to train with replay buffers, expert data warm-starting, and mixed on/off-policy batches.
+ [Exploration Strategies](https://torchgfn.readthedocs.io/en/latest/guides/exploration_strategies.html): Epsilon-greedy, temperature scaling, noisy layers, local search, and variance annealing for mode discovery.
+ [Conditional GFlowNets](https://torchgfn.readthedocs.io/en/latest/guides/conditional_gflownets.html): Learning families of distributions parameterized by external conditions.
+ [Recurrent & Non-Autoregressive Policies](https://torchgfn.readthedocs.io/en/latest/guides/recurrent_policies.html): LSTM/GRU policies, tree DAG simplifications, and order-invariant generation.
+ [Diffusion GFlowNets](https://torchgfn.readthedocs.io/en/latest/guides/diffusion_gflownets.html): Continuous diffusion sampling with Brownian motion estimators and RTB fine-tuning.
