#!/usr/bin/env python
"""Profile torchgfn HyperGrid environment to identify compute bottlenecks.

Usage:
    python benchmark/profile_hypergrid.py [--n-iterations 50] [--batch-size 16]
"""
    ...
    with timer("total"):
        ...
        with timer("loss"):
            ...
            loss = gflownet.loss_from_trajectories(
                env,
                trajectories,
                recalculate_all_logprobs=True,  # Hypergrid recalculates
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Profile torchgfn HyperGrid environment"
    )
    ...
    optimizer.add_param_group({"params": gflownet.logz_parameters(), "lr": 0.1})
    ...
    # Profile
    print(
        f"Profiling ({args.n_iterations} iterations, batch_size={args.batch_size}, "
        f"ndim={args.ndim}, height={args.height})..."
    )
    timer = Timer()
