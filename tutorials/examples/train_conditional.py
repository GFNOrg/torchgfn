...
class ConditionalHyperGrid(HyperGrid):
    ...
    def reward(self, states: DiscreteStates) -> torch.Tensor:
        ...
        max_reward = (self.reward_fn_kwargs.get("R0", 0.1) + 
...
def train(env, gflownet, seed, device, n_iterations=10, batch_size=1000, 
          validation_interval=100, validation_samples=200000, lr=1e-3, lr_logz=1e-2, epsilon=0.0):
    ...
    print(f"+ Environment: ndim={env.ndim}, height={env.height}, n_states={env.n_states}")
    print(f"+ Training parameters: n_iter={n_iterations}, batch_size={batch_size}, lr={lr}, epsilon={epsilon}")
    ...
    for it in (pbar := tqdm(range(n_iterations), dynamic_ncols=True)):
        ...
        if (it + 1) % validation_interval == 0:
            ...
            for cond_val in test_cond_values:
                ...
                conditioning_val = torch.full((validation_samples, 1), cond_val, device=device)
                ...
                with torch.no_grad():
                    ...
                    sampled_states = cast(DiscreteStates, sampled_trajectories.terminating_states)
            ...
            print(f"Iter {it + 1}: L1=[{l1_dists[0]:.6f}, {l1_dists[1]:.6f}, {l1_dists[2]:.6f}, {l1_dists[3]:.6f}, {l1_dists[4]:.6f}] modes={len(discovered_modes)}")
...
def evaluate_conditional_sampling(env, gflownet, device, n_eval_samples=10000):
    ...
    for cond_value in test_cond_values:
        ...
        conditioning = torch.full((n_eval_samples, 1), cond_value, dtype=torch.float, device=device)
        env.set_conditioning(conditioning)
        
        # Sample without exploration
        print(f"Sampling {n_eval_samples} trajectories with conditioning={cond_value}...")
        trajectories = gflownet.sample_trajectories(
            env,
            n=n_eval_samples,
            conditioning=conditioning,
        ...
        print(f"  Top {min(5, top_k)} empirical probs: {[f'{p:.4f}' for p in emp_topk_vals[:5].tolist()]}")
        print(f"  Top {min(5, top_k)} true probs:      {[f'{p:.4f}' for p in true_topk_vals[:5].tolist()]}")
        ...
        if cond_value == 0.0:
            ...
            print(f"\nUniformity Metrics:")
            print(f"  Max probability: {max_prob:.6f} (expected: {expected_uniform_prob:.6f})")
            print(f"  Min probability: {min_prob:.6f} (expected: {expected_uniform_prob:.6f})")
            ...
            print(f"  Probability mass on significant states (p>0.01): {mode_prob_mass:.4f}")
            ...
            kl_div = (empirical_dist * (empirical_dist + eps).log() - empirical_dist * (true_dist + eps).log()).sum().item()
    ...
    print("="*60)
    for cond_val in test_cond_values:
        print(f"Conditioning={cond_val:.2f}: L1 distance = {results[cond_val]['l1_dist']:.6f}")
    ...
    if args.gflownet == "all":
        ...
        for fn_name, fn in GFN_FNS.items():
            ...
            final_loss = train(
                ...
                validation_interval, args.validation_samples, args.lr, args.lr_logz, args.epsilon
            )
