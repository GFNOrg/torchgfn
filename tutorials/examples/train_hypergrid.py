...
def set_up_fm_gflownet(args, env, preprocessor, agent_group_list, my_agent_group_id):
    ...
    estimator = DiscretePolicyEstimator(
        module=module,  # type: ignore  # FIXME
...
def set_up_pb_pf_estimators(
    ...
    pf_estimator = DiscretePolicyEstimator(
        module=pf_module,  # type: ignore  # FIXME
        n_actions=env.n_actions,
        preprocessor=preprocessor,
    )
    pb_estimator = DiscretePolicyEstimator(
        module=pb_module,  # type: ignore  # FIXME
        n_actions=env.n_actions,
        is_backward=True,
        preprocessor=preprocessor,
    )
...
def set_up_logF_estimator(
    ...
    return ScalarEstimator(module=module, preprocessor=preprocessor)  # type: ignore  # FIXME
    ...
    for iteration in pbar:
        ...
        if replay_buffer is not None:
            with torch.no_grad():
                ...
                training_objects = replay_buffer.sample(n_samples=per_node_batch_size)
        else:
